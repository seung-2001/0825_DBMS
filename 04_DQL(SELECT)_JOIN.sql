...
-->  WHERE 절에서는 매칭을 시키고자 하는 컬럼명에 대한 조건을 제시함( = )
...
-- DEPT_ID가 D3, D4, D7인 부서 데이터가 조회되지 않는다.
...
-- 사원명, 부서코드, 부서명
-- 연결컬럼이 컬러명이 다름(EMPLOYEE - DEPT_CODE / DEPARTMENT - DEPT_ID)
...
-- 사원명, 직급코드, 직급명
-- 연결할 두 컬럼명이 같은경우 (JOB_CODE)
-- ON 구문이용 ) 애매하다(AMBIGOUSLY) 발생할 수 있음 어떤 테이블의 컬럼인지 명시
SELECT 
	   EMP_NAME
	 , E.JOB_CODE
	 , JOB_NAME
...
-- USING구문 이용 ) 컬럼명이 동일할 경우 사용이 가능하며 동등비교연산자를 기술하지 않음
SELECT 
	   EMP_NAME
	 , JOB_CODE
	 , JOB_NAME
...
-- 조건과는 상관없이 왼편에 기술한 테이블의 데이터는 전부 조회(일치하는 값을 못 찾더라도 조회)
...
--> ORACLE : FULL [ OUTER ] JOIN 오라클 구문에서는 사용이 불가능!!
SELECT 
	   EMP_NAME
	 , DEPT_TITLE
  FROM
...
SELECT * FROM DEPARTMENT; -- DEPT_TITLE  DEPT_ID    	      LOACTION_ID
 ...
 * - INTERSCET : 교집합(두 쿼리문의 수행 겨롸값 중 중복된 부분)
...
---------------------------------------------------------------------------
-- 4. MINUS (차집합 - 선행쿼리 결과에서 우행 쿼리결과를 뺸 나머지)
SELECT
	   EMP_NAME
	 , DEPT_CODE
     , SALARY
