 ...
 *                     고정 길이(아무리 작은 값이 들어와도 공백으로 채워서 크기 유지)
 *                    주로 들어올 값의 글자수가 정해져 있을 경우
 *                    예) 성별 M/F
 * - VARCHAR2(바이트크기) : VAR는 '가변'을 의미, 2는 2배를 의미
 *                       최대 4000BYTE까지 지정 가능
 *                       가변 길이(적은 값이 들어오면 크기가 줄어듬)
 *                       --> CLOB, BLOB
 ...
 * 
 */
COMMENT ON COLUMN TB_USER.USER_ID IS '회원아아디';
...
INSERT INTO TB_USER VALUES('user01', 'pass01', '강현성', '25/08/27');
 ...
 * < 제약 조건 CONSTRAINT >
 * 
 * - 테이블에 유효한 데이터값만 유지하기 위해서 특정 컬럼마다 제약 == 데이터 무결성 보장을 목적
...
-- NULL을("KHS02"."TB_USER_NOT_NULL"."USER_NO") 안에 삽입할 수 없습니다
 INSERT 
   INTO
        TB_USER_NOT_NULL
 VALUES
...
CREATE TABLE TB_USER_UNIQUE (
	...
	USER_ID VARCHAR2(20) NOT NULL,       -- NULL UNIQUE,
	USER_PWD VARCHAR2(20) NOT NULL,
	USER_NAME NVARCHAR2(20),
	GENDER CHAR(3),
	CONSTRAINT ID_UNIQUE UNIQUE(USER_ID) -- 테이블레벨방식
);
...
-- 제약조건명을 달아두면 문제가 생긴 원일은 조금 더 쉽게 유추할 수 있음
INSERT INTO TB_USER_UNIQUE VALUES(3, 'user01', '1234', '홍길동', '밥');
SELECT * FROM TB_USER_UNIQUE;
-- GENDER 컬럼에는 '남' 또는 '여'라는 값만 들어갈 수 있게 하고 싶음
-----------------------------------------------------------------------------------
/*
 * 3. CHECK 제약조건
...
-- CHECK를 부여하더라도 NULL 값은 INSERT할 수 있음 -> NOT NULL로 막아야함
-----------------------------------------------------------------------------------
/*
 * 테이블 만들기 실습
...
-- 특정 컬럼에 기본값을 설정하는 방법 ===> 제약조건은 아님
 CREATE TABLE TB_USER_DEFAULT (
 	...
 	USER_NICKNAME VARCHAR2(10) UNIQUE,
 ...
 * 참조하는 테이블에 존재하는 값만 INSERT할 수 있음
 * => FOREING KEY 제약조건을 이용해서 다른 테이블간의 관계를 형성할 수 있음
 * 
 * [ 표현법 ]
 * - 컬럼 레벨 방식
 * 컬럼명 자료형 REFERENCES 참조할테이블명  -- PRIMARY KEY 가 있을 경우
 * 컬럼명 자료형 REFERENCES 참조할테이블명 (참조할컬럼명) -- PRIMARY KEY 가 없을 경우
 * 
 * - 테이블 레벨 방식
 * FOREIGN KEY (컬럼명) REFERENCES 참조항테이블명 (참조할컬럼명)
...
CREATE TABLE TB_USER_CHILD ( -- 자식테이블
	...
	USER_ID VARCHAR2(15) NOT NULL UNIQUE,
	USER_PWD VARCHAR2(20) NOT NULL,
	GRADE_ID CHAR(2) REFERENCES USER_GRADE -- 컬럼 레벨 방식
...
ALTER TABLE JOB_COPY MODIFY JOB_CODE CHAR(4);
...
-- 1_3) 컬럼 삭제(DROP COLUIN) DROP COLUMN 컬럼명
