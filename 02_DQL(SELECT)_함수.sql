...
SELECT 
  ...
  FROM
  	   DUAL; -- 뒤에서 부터 첫 번쨰 'B'가 앞에서부터 몇 번째인지
  	   
SELECT 
	   INSTR('AABAACAABBAA', 'B', 1, 3) 
  FROM
       DUAL; -- 앞에서 부터 세 번쨰 'B'가 앞에서부터 몇 번째인지
...
-- EMPLOYEE 테이블로부터 사원명과 이메일컬럼과 EMAIL컬럼값 아이디값만 추출해서 조회
SELECT 
       EMP_NAME
     , EMAIL
...
SELECT
	   RPAD(SUBSTR(EMP_NAME, 1, 1), LENGTH(EMP_NAME)+1,'*')
...
-- EMPLOYEE테이블로부터 각 사원 사원명, 고용일로부터 근무일수
...
-- ADD_MONTH(DATE, NUMBER) : 특정날짜에 해당 숫자만큼의 개월 수를 더한 날짜(DATE로 반환)
SELECT 
...
SELECT 
	 ...
	 , TO_CHAR(SYSDATE, 'RRRR') -- ROUNDYEAR
...
-- 직그코드가 'J6'인 사원들의 급여는 20%인상해서 조회
-- 직그코드가 'J5'인 사원들의 급여는 30%인상해서 조회
...
-- 현재 사원들이 속해있는 부서 개수
-- COUNT(DISTINCT 컬럼명) : 제시한 컬럼값이 중복값이 존재할경우 하나로만 세서 반환
SELECT
